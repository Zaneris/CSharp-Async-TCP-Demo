...
namespace Networking.Packets {
    public class PacketBuilder {
        ...
        public (bool Complete, int Size) BuildPacket(byte[] buffer, IPacket packet) {
            ...
            if(_lastSendByte == 0) { // Starting with new data.
                ...
                } else { // All bytes will not be sent.
            ...
            } else { // Last packet isn't finished yet.
        ...
        public List<IPacket> ReceivePacket(byte[] bytes, int length, List<IPacket> list) {
            int i = 0;
            list.Clear();
            while(i < length) {
                ...
                if(_buffer is null && packetLength <= bytes.Length - HEADER_SIZE) {
                    ...
                    if(_buffer is null) {
                        ...
                        return list;
                    } else if(length >= _remainingBytes) {
